<div class="container edit-user">
    <div class="col-12 col-md-12">
        <form action="<?='http://'.HOST.'/user/update-profile'?>" class="form" method="POST">
                                ...
                                <div class="form-group">
                                    <input type="text" class="form-control" name="name" id="name" placeholder="Your name ..." require pattern="^[a-zA-Z_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ\\s]+$" value="<?php if (isset($user["name"])) echo ($user["name"]) ?>">
                                ...
                                <div id="login-form-action">
                                    &emsp;
                                    <button type="submit" class="btn btn-info btn-block btn-round" id="submit" onclick="return editUserProfileBehavior()"> Register Now </button>
                                    <br><br>
                                </div>
